...
### ORM 與持久層框架 (Hibernate / Spring Data JPA / MyBatis)

#### 名詞解釋

- **JPA (Java Persistence API)**：Java 持久層的**規範（介面）**，只定義介面與註解（如 `@Entity`、`@Id`、`@OneToMany`），本身不含實作。需搭配實作框架（如 Hibernate）才能運作。
- **Hibernate**：JPA 最主流的**實作**，是一套全自動 **ORM (Object-Relational Mapping)** 框架，負責把 Java 物件與資料表自動對應、自動生成 SQL。也可脫離 JPA 規範使用其原生 API。
- **Spring Data JPA**：Spring 對 JPA 的**再封裝**。它本身**不做 ORM**，底層預設仍是 Hibernate；作用是省去手寫 DAO/Repository 實作，透過繼承 `JpaRepository` 與「方法名衍生查詢」自動產生 CRUD。
- **MyBatis**：半自動的 **SQL Mapper**（非 ORM）。開發者**自行撰寫 SQL**，MyBatis 負責把參數綁定與結果集映射成物件。

#### 三者關係

```
JPA (規範 / 介面)
  └── Hibernate (JPA 的實作,ORM 框架)
         └── Spring Data JPA (在 Hibernate 之上再封裝,簡化 Repository)

MyBatis (獨立陣營,SQL Mapper,手寫 SQL)
```

> 重點：**Spring Data JPA + Hibernate 屬同一條技術線（全自動 ORM）**；**MyBatis 是另一條線（手寫 SQL）**。三者不在同一層級。

#### 核心差異比較

| 面向 | Hibernate / Spring Data JPA | MyBatis |
|------|------|---------|
| 類型 | 全自動 ORM | 半自動 SQL Mapper |
| SQL 來源 | 框架自動生成 | 開發者手寫 |
| 學習曲線 | 較陡（快取、持久化狀態、延遲載入） | 平緩（會 SQL 即可上手） |
| SQL 掌控度 | 低（被框架封裝） | 高（完全掌控） |
| CRUD 開發效率 | 高（幾乎免寫 SQL） | 低（每個方法都要寫） |
| 複雜查詢 / 報表 | 不擅長（易生低效 SQL） | 擅長 |
| 跨資料庫移植 | 強（自動適配方言 Dialect） | 弱（SQL 易寫死） |
| 一級 / 二級快取 | 內建 | 需自行整合 |
| 效能調優 | 較難（SQL 不透明） | 容易（SQL 可見） |

#### 程式碼對比

```java
// Spring Data JPA：連 SQL 都不用寫,方法名即查詢
public interface UserRepository extends JpaRepository<User, Long> {
    List<User> findByAgeGreaterThanAndStatus(int age, String status);
}

// Hibernate (JPQL)：物件導向查詢語言
...
public interface UserMapper {
    @Select("SELECT * FROM user WHERE age > #{age} AND status = #{status}")
    List<User> findUsers(@Param("age") int age, @Param("status") String status);
}
```
...
| 情境 | 建議 |
|------|------|
| 標準 CRUD 為主、業務單純、重視開發速度 | Spring Data JPA + Hibernate |
| 複雜報表、多表 JOIN、需極致 SQL 效能調優 | MyBatis |
| 團隊 SQL 能力強、想完全掌控 SQL | MyBatis |
| 兼顧 | 混用：JPA 做 CRUD、MyBatis 做複雜查詢 |

#### 面試常見追問

- **JPA / Hibernate / Spring Data JPA 三者關係** → 規範 / 實作 / 封裝。
- **N+1 查詢問題** → Hibernate 延遲載入經典坑，用 `JOIN FETCH` 或 `@EntityGraph` 解。
- **一級快取 vs 二級快取** → Session 級 vs SessionFactory 級。
- **持久化狀態** → transient / persistent / detached，及 dirty checking 機制。
- **為何大型專案常選 MyBatis** → SQL 透明、好調效能、DBA 友善。
