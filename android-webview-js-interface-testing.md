...
如果不方便使用 Chrome DevTools，可以建立一個本地 HTML 測試頁面：
...
<body>
    <h2>JavaScript Interface Test</h2>
    <button style="background:#4CAF50" onclick="window.AndroidShow.showSoundStateToast('false')">
        showSoundStateToast(false) - 聲音已開啟
    </button>
    <button style="background:#f44336" onclick="window.AndroidShow.showSoundStateToast('true')">
        showSoundStateToast(true) - 聲音已關閉
    </button>
    <button style="background:#ff9800" onclick="window.AndroidShow.onForceLogout()">
        onForceLogout - 強制登出
    </button>
</body>
</html>
...
```bash
adb push test_bridge.html /sdcard/Download/test_bridge.html
```

2. 用 App 的 WebActivity 開啟：

```bash
adb shell am start -n com.showyouapp.ekkorn/com.work.xux.want.presentation.feature.web.WebActivity \
    -e web_url "file:///sdcard/Download/test_bridge.html"
```

3. 點擊頁面上的按鈕即可觸發對應的原生方法

> **注意**：此方式開啟的 WebView 會注入通用的 `JS2NativeInterface`，但某些行為可能與特定頁面（如 Minikorn 聊天室）不同，因為部分邏輯依賴 `explicitPageId` 參數。
