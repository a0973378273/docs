...
```
┌────────┐   ┌────────────┐   ┌────────────┐   ┌──────────┐
│Request │──→│Interceptor1│──→│Interceptor2│──→│ 實際執行  │
└────────┘   │(加 Header) │   │(加 Log)    │   └──────────┘
             └────────────┘   └────────────┘
```
...
```
┌─────────────────────────────────────────┐
│            Presentation Layer           │  ← 最外層：UI、ViewModel
│  ┌─────────────────────────────────┐    │
│  │          Domain Layer           │    │  ← 中間層：UseCase、Model
│  │  ┌─────────────────────────┐    │    │
│  │  │      Data Layer         │    │    │  ← 最內層概念上，但依賴反轉
│  │  └─────────────────────────┘    │    │     讓 Data 實作 Domain 介面
│  └─────────────────────────────────┘    │
└─────────────────────────────────────────┘
```
...
```
              Android              iOS                Flutter
            ┌─────────┐       ┌──────────┐        ┌──────────┐
Async       │Coroutines│       │async/await│        │ Future   │
            │  Flow    │       │ Combine   │        │ Stream   │
            ├─────────┤       ├──────────┤        ├──────────┤
Network     │ Retrofit │       │ URLSession│        │   dio    │
            │ OkHttp   │       │ Alamofire │        │   http   │
            ├─────────┤       ├──────────┤        ├──────────┤
DI          │  Hilt    │       │ Swinject  │        │  get_it  │
            │  Koin    │       │ 手動 DI    │        │injectable│
            ├─────────┤       ├──────────┤        ├──────────┤
Local DB    │  Room    │       │ CoreData  │        │ sqflite  │
            │DataStore │       │ Realm     │        │  Hive    │
            ├─────────┤       ├──────────┤        ├──────────┤
Image       │  Coil    │       │Kingfisher │        │cached_net│
            │ Glide    │       │ SDWebImage│        │_image    │
            ├─────────┤       ├──────────┤        ├──────────┤
Navigation  │  NavComp │       │ NavStack  │        │ GoRouter │
...
```
MVC → MVP → MVVM → MVI (單向資料流)
                         │
                    Clean Architecture（可搭配任何展示層模式）
```
...
```
需要可預測的狀態管理？
├── 是 → MVI / TCA / BLoC（單向資料流）
│        └── 搭配 Clean Architecture 分層
└── 否 → MVVM 就夠用
         └── 狀態簡單的頁面不需要過度設計
```
...
```
┌─────────────────────────────────────────────┐
│ Presentation: MVI / TCA / BLoC              │
│   Event → ViewModel/Store/Bloc → State → UI │
├─────────────────────────────────────────────┤
